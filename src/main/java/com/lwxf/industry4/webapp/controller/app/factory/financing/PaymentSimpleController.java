...
import com.lwxf.industry4.webapp.facade.app.factory.financing.PaymentSimpleFacade;
...
@Api(value="PaymentSimpleController",tags={"F端APP接口:财务日常账管理"})
@RestController("fpaymentSimpleControllerApp")
@RequestMapping(value = "/app/f/", produces = WebConstant.RESPONSE_CONTENT_TYPE_JSON_CHARTSET)
public class PaymentSimpleController {
    @Resource(name = "paymentSimpleFacadeFapp")
    ...
    public String getUserForPaymentSimple(HttpServletRequest request){
        JsonMapper jsonMapper=new JsonMapper();
        String atoken=request.getHeader("X-ATOKEN");
        MapContext mapInfo = LoginUtil.checkLogin(atoken);
        String uid =mapInfo.get("userId")==null?null:mapInfo.get("userId").toString();
