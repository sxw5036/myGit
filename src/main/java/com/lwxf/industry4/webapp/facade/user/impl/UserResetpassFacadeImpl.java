...
import com.lwxf.industry4.webapp.facade.user.UserResetpassFacade;
...
import static com.lwxf.industry4.webapp.facade.AppBeanInjector.userFacade;
...
//		tsManualData.setEvent(JMailService.MAILSEND_CLASSNAME_RESETPASSWORDMAIL);
	...
	public RequestResult saveNewPass(String code, String password,MapContext mapContext) {
		...
		if (b) {
			...
			update.put(WebConstant.KEY_ENTITY_USER_ID,user.getId());
			update.put(WebConstant.KEY_ENTITY_APP_TOKEN,UserExtraUtil.generateAppToken(userExtra,null));
			AppBeanInjector.userThirdInfoService.updateByMapContext(update);
		}
		userResetpassService.deleteById(userResetpass.getId());
