...
import static com.lwxf.industry4.webapp.facade.AppBeanInjector.*;
...
public class UserThirdInfoFacadeImpl extends BaseFacadeImpl implements UserThirdInfoFacade {
	...
	@Override
	@Transactional(value = "transactionManager")
	public void updateRongToken(String userId, String rongToken) {
		if(StringUtils.isBlank(userId) || StringUtils.isBlank(rongToken)){
			return;
		}
		MapContext update = MapContext.newOne();
		update.put(WebConstant.KEY_ENTITY_USER_ID,userId);
		update.put(WebConstant.KEY_ENTITY_RONGCLOUD_TOKEN,rongToken);
