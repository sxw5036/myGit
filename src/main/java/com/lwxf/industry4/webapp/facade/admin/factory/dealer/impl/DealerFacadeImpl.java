...
public class DealerFacadeImpl extends BaseFacadeImpl implements DealerFacade {
	...
	public RequestResult addDealer(CompanyDto company) {
		...
		if(company.getNo()!=null&&this.companyService.selectByNo(company.getNo())!=null){
	...
	@Override
	@Transactional(value = "transactionManager")
	public RequestResult updateDealerCompany(MapContext mapContext, String cid) {
		//判断公司是否存在
		...
		if(no!=null){
			Company company = this.companyService.selectByNo(no);
	...
	public RequestResult findDealerList(MapContext mapContext, Integer pageNum, Integer pageSize) {
		...
		pagination.setPageSize(pageSize);
		paginatedFilter.setPagination(pagination);
		mapContext.put("roleId",this.roleService.selectByKey(DealerEmployeeRole.SHOPKEEPER.getValue()).getId());
