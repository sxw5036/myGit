...
public abstract class ShiroUtil {
	...
	public final static LoginedUser getCurrUser() {
		String userId = getCurrUserId();
		if (userId == null) {
			return null;
		}
		return AppBeanInjector.userFacade.findLoginedUserById(userId);
	}
	...
	private static final Pattern pathPattern = Pattern.compile("/(mall(/#/(trolley|user|order/\\d))?|admin|storecard|quickshare|reservation|payment(/mppay)?|wxpay/notify)");
